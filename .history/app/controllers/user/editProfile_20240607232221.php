...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        if ($avatarUrl) {
            $stmt = $conn->prepare("UPDATE users SET username = :username, email = :email = = :last_name, avatar_url = :avatar_url WHERE id = :user_id");
            $stmt->bindParam(':avatar_url', $avatarUrl, PDO::PARAM_STR);
        } else {
            $stmt = $conn->prepare("UPDATE users SET username = :username, email = :email = last_name = :last_name WHERE id = :user_id");
        }
